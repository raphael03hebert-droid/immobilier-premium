(() => {
  ...
  function addSyncButton() { const inbox = document.querySelector('.inbox'); if (!inbox) return; if (!inbox.dataset.emailLive) { inbox.dataset.emailLive = '1'; inbox.innerHTML = '<div class="empty-note">Cliquez sur « Charger ma boîte » pour afficher vos vrais courriels Outlook.</div>'; } if (inbox.previousElementSibling?.classList.contains('email-sync-head')) return; const head = document.createElement('div'); head.className = 'email-sync-head'; head.innerHTML = '<div><b>Microsoft Outlook</b><small>Boîte de réception et brouillons synchronisés.</small></div><button class="btn" type="button" data-action="load-email">Charger ma boîte</button>'; inbox.parentElement.insertBefore(head, inbox); }
