(() => {
  ...
  async function send(value) {
    ...
    try {
      const response = await fetch('/api/ai-chat', { method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({messages:state.messages.slice(-24)}), signal:state.controller.signal });
